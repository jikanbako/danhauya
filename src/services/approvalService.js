...
/*
 * --------------------------------------------------
 * CONSOLIDATED SOURCE WORKFLOW SYNCHRONIZATION
 * --------------------------------------------------
 *
 * A consolidated requisition is the representative workflow record,
 * but every source requisition remains visible to its requester. This
 * helper mirrors the representative's completed approval into each
...
export async function returnStep({
  ...
  const returnedToProcurementReview =
    requisition.approvalChain?.[requisition.currentStepIndex]?.role === ROLES.PROCUREMENT &&
    requisition.approvalChain?.[requisition.currentStepIndex]?.type === "procurement_review";

  /*
   * VC -> Procurement is a workflow return, not a requester return.
   * Keep the requisition pending at Procurement and preserve the same
   * assigned officer so the market survey can be revised and submitted again.
   */
  requisition.status = returnedToProcurementReview
    ? REQUISITION_STATUS.PENDING
    : REQUISITION_STATUS.RETURNED;

  if (returnedToProcurementReview) {
    requisition.procurementStatus = "review";
    requisition.awaitingRequesterAction = false;
    requisition.procurementReceivedAt ||= new Date();
  } else if (requisition.procurementStatus) {
    requisition.procurementStatus = undefined;
    requisition.procurementOfficer = undefined;
    requisition.procurementReceivedAt = undefined;
  }
...
export async function rejectStep({
  ...
  /*
   * A final VC rejection remains visible to the Procurement staff who
   * handled the market survey. Preserve the assigned officer and mark
   * the Procurement outcome as rejected. Non-final rejection keeps the
   * existing requester-return behavior.
   */
  if (isFinal && step.role === ROLES.VC && requisition.procurementOfficer) {
    requisition.procurementStatus = "rejected";
    requisition.procurementReceivedAt ||= new Date();
    requisition.procurementStartedAt = undefined;
    requisition.procurementCompletedAt = undefined;
  } else {
    requisition.procurementStatus = undefined;
    requisition.procurementOfficer = undefined;
    requisition.procurementReceivedAt = undefined;
  ...
  if (isFinal && step.role === ROLES.VC) {
    await syncConsolidatedSourcesAfterRejection({
      representative: requisition,
      rejectingStep: step,
      rejectorId: approverUser.id,
      comment,
    });
  }
...
/*
 * --------------------------------------------------
 * CONSOLIDATED SOURCE REJECTION SYNCHRONIZATION
 * --------------------------------------------------
 *
 * The consolidated requisition is the representative workflow record.
 * A final VC rejection must therefore be mirrored to every source so
 * their requesters/HODs never see a stale Procurement-pending state.
 */
async function syncConsolidatedSourcesAfterRejection({
  ...
  for (const source of sources) {
    const vcIndex = source.approvalChain?.findIndex(
      (step) => step.role === ROLES.VC && step.type === "approval"
    );

    if (vcIndex >= 0) {
      source.currentStepIndex = vcIndex;
    }
    ...
    if (representative.procurementOfficer && !source.procurementOfficer) {
      source.procurementOfficer = representative.procurementOfficer;
    }

    const rejectionMessage =
      comment ||
      `Rejected through consolidated requisition ${representative.requisitionNumber || representative._id}.`;
    ...
    await AuditLog.create({
      ...
      details: {
        representativeRequisition: representative._id,
        rejectedRole: rejectingStep.role,
        comment: rejectionMessage,
      },
    });
