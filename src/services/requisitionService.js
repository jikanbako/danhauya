...
/*
    ...
    if (
      ...
      if (requisition.isConsolidated) {
        // A consolidated requisition inherits its organizational
        // units from its source requisitions. Never recalculate it
        // from the normal single-requester organization picker.
        requisition.isConsolidated = true;
        requisition.requestingUnits =
          requisition.requestingUnits || [];
        // Keep the existing top-level snapshot (which may be N/A
        // when the consolidation spans multiple units).
      } else {
...
export async function submitRequisition({
  ...
  if (canPickOrganization && !requisition.isConsolidated) {
    const units =
      requisition.requestingUnits ||
      [];
  ...
  // Consolidated requisitions already carry their organizational
  // units from the source requisitions. We do not ask the
  // consolidator to select a department again. We only verify that
  // multi-unit items still point to one of those inherited units.
  if (requisition.isConsolidated) {
    const units = requisition.requestingUnits || [];
    if (units.length === 0) {
      throw new Error(
        "This consolidated requisition has no requesting units from its source requisitions."
      );
    }

    if (units.length > 1) {
      const validKeys = new Set(units.map(unitKey));
      const untaggedItem = requisition.items.find((item) => {
        const key = [
          item.requestingCollegeId,
          item.requestingFacultyId,
          item.requestingDepartment,
      ...
      if (untaggedItem) {
        throw new Error(
          "A consolidated item is missing its originating department."
        );
      }
    }
  }
